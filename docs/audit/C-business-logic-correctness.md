...
| C-19 | Поставляемая ревизия не генерируется с документированным минимумом полей | High | Подтверждено трассировкой |
...
Вызов на `:246` соответственно перед��ёт `sagaID`. Дополнительно — сделать `IdempotencyKey` обязательным на уровне HTTP-контракта для мутирующих операций (см. раздел B).
...
## C-19. Единственная поставляемая ревизия не генерируется с документированным минимумом полей

**Суть.** `requiredInputFields` объявляет минимум из 3 полей, но шаг цепочки `DAK` зависит от 4 полей, которых в этом минимуме нет и которые в схеме помечены `required: false`. Запрос с ровно документированным минимумом проходит валидацию и падает на цепочке. Отдельно: `eyeColor` помечен в схеме `required: true`, но BFF его не требует.

**Место.** `configs/revisions/US_CA_08292017.yaml:5-15,75-80`; `internal/usecase/generate_usecase.go:107` (`validateMinimumSet`), `:163` (`chain.Execute`), `:521-532`.

**Подробное описание.**

Конфигурация единственной ревизии в репозитории:
...
calculationChain:
  - field: DAK
    source: calculate
    dependsOn: [street, city, state, zipCode]   # ← ни одного нет в requiredInputFields
```

При этом в `schema.fields` все четыре поля адреса объявлены `required: false`, а `eyeColor` — `required: true`.

`validateMinimumSet` проверяет **только** `RequiredInputFields`:
...
Порядок в `Generate`: `:107` валидация → `:163` цепочка → `:230` `Block`. Трассировка запроса с документированным минимумом:

1. `:107` — `firstName`, `lastName`, `dateOfBirth` заполнены → `nil`, проходит.
2. `:163` — шаг `DAK` требует `street`/`city`/`state`/`zipCode`, ни одного нет → `MISSING_DEPENDENCY`.
3. `:230` `Block` не достигается.

Денег это не теряет — падение до списания. Но означает, что **основной сценарий единственной поставляемой ревизии нерабочий**: клиент, заполнивший всё, что сервис объявил обязательным, гарантированно получает ошибку. Чтобы запрос прошёл, нужно заполнить 4 поля, которые схема объявляет необязательными, то есть контракт схемы противоречит фактическому требованию цепочки.

Второе расхождение — в обратную сторону. `eyeColor` имеет `required: true` в схеме, но отсутствует в `requiredInputFields`, а `validateMinimumSet` читает только второй список. Фронтенд, рендерящий форму по `GET /revisions/:revision/schema`, покажет поле обязательным; BFF примет запрос без него и отправит в BarcodeGen документ без цвета глаз. Для AAMVA-элемента это порча выходных данных, а не косметика.

Итого один YAML содержит три несогласованных представления обязательности: `requiredInputFields` (что проверяет BFF), `schema.fields[].required` (что показывает фронтенд), `calculationChain[].dependsOn` (что нужно фактически).

**Предлагаемый багфикс.** Вывести обязательность из одного источника и провалидировать конфиг при загрузке. В `validateRevisionConfig` (см. C-10) добавить:

```go
// Транзитивная обязательность: всё, от чего зависят шаги цепочки и что
// не вычисляется самой цепочкой, должно быть в RequiredInputFields.
computed := make(map[string]bool, len(cfg.CalculationChain))
for _, s := range cfg.CalculationChain {
    computed[s.Field] = true
}
required := make(map[string]bool, len(cfg.RequiredInputFields))
for _, f := range cfg.RequiredInputFields {
    required[f] = true
}
for _, s := range cfg.CalculationChain {
    for _, dep := range s.DependsOn {
        if !computed[dep] && !required[dep] {
            return fmt.Errorf(
                "revision %s: поле %q — зависимость шага %q, но не объявлено в requiredInputFields",
                cfg.Name, dep, s.Field)
        }
    }
}
// Обратная сверка со схемой.
for _, f := range cfg.Schema.Fields {
    if f.Required && !required[f.Name] && !computed[f.Name] {
        return fmt.Errorf(
            "revision %s: поле %q имеет schema.required=true, но не входит в requiredInputFields",
            cfg.Name, f.Name)
    }
}
```

Затем привести сам YAML в согласованное состояние — выбрать одно из двух:

- поля адреса действительно обязательны → добавить их в `requiredInputFields` и поставить `required: true` в схеме;
- поля адреса опциональны → сделать `DAK` условным шагом, выполняемым только при их наличии (потребует поддержки опциональных шагов в `ChainExecutor`).

Плюс добавить `eyeColor` в `requiredInputFields` либо снять `required: true` в схеме.

**Комментарий.** Единственный дефект раздела, найденный не в коде, а в данных, — и потому не покрытый ни одним тестом: тесты `ChainExecutor` используют собственные inline-конфиги, а не поставляемый YAML. Пока `validateRevisionConfig` отсутствует, любая правка ревизии через `PUT /admin/revisions/:revision` может внести такое же расхождение, и обнаружится оно только на живом трафике. Первопричина общая с C-10 и C-18: обязательность полей размазана по трём независимым спискам без единого валидатора.
