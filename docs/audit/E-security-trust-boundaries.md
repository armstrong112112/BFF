...
| E-21 | `JWT_SECRET` отсутствует в `.env.example` и compose — дефолт неперекрываем | 🔴 Критическая | Подтверждено сверкой конфигурации |
...
5. `PublishBarcodeEdited{ID: barcodeID, NewURL: ...}` — History Service по этому событию
   ставит `editFlag=true` и **обн��вляет `imageUrl` записи жертвы**.
...
Логи при этом выгля��ят «нормально»: одна информационная строка при старте, никаких ошибок.
...
как одного» — меняет модель безопасности и модель биллинга одновременно. Правильная деград��ция
...
исправления се��рета отсутствие `exp` оставляет невозможность отзыва.
...
1. **��дентификаторы и URL артефактов.** `barcodeUrl` — прямая ссылка на изображение документа.
...
Если разделить порты не��ьзя — закрыть тем же service-токеном:
...
## E-21. `JWT_SECRET` отсутствует в `.env.example` — штатного способа перекрыть дефолт нет

### Суть
`JWT_SECRET` имеет рабочий дефолт `dev-jwt-secret` (E-03), но **не упомянут** ни в `.env.example`, ни в `docker-compose`. Оператор, настраивающий сервис по документации, не имеет способа узнать о существовании этой переменной — и гарантированно уезжает в production с дефолтным секретом подписи пользовательских токенов.

### Место
`internal/config/config.go:134` (дефолт), `.env.example` (отсутствует), `docker-compose.yml` / `docker-compose.prod.yml` (отсутствует).
...
Проверка присутствия в документируемой конфигурации:

```
$ grep -n "JWT_SECRET" .env.example docker-compose*.yml
(пусто)
```

При этом `.env.example` аккуратно перечисляет 24 других переменных, включая менее критичные (`GRAFANA_ADMIN_PASSWORD`, `IDEMPOTENCY_TTL`, все таймауты) и однокоренные `INTERNAL_SERVICE_JWT` и `ADMIN_JWT`. То есть пропуск не является следствием общей неполноты файла — отсутствует именно самый критичный секрет из трёх.

Почему это отдельный дефект, а не повтор E-03. E-03 фиксирует наличие небезопасного дефолта: он эксплуатируем, если оператор не переопределил переменную. E-21 фиксирует, что переопределить её **невозможно в рамках документированного процесса**: `.env.example` — единственный артефакт, описывающий конфигурацию (`README` отправляет к нему, `deploy/` не существует, см. F-18). Оператор, добросовестно заполнивший все поля из примера, получает работающий сервис, в котором подпись пользовательских JWT проверяется публично известным секретом из git-истории. E-03 — «можно забыть»; E-21 — «узнать нечего».

Практический сценарий: `make prod-up` поднимает стек по `docker-compose.prod.yml`, где `JWT_SECRET` не объявлен, поэтому в контейнер он не пробрасывается даже если задан в `.env`. Значит, дефолт применяется **даже при корректно заполненном `.env`** — переменная не дойдёт до процесса без правки compose-файла. Это делает E-03 не потенциальным, а фактическим состоянием prod-like развёртывания.

Последствие совпадает с E-03: подделка токена любого пользователя, что в сочетании с E-01/E-02 даёт чтение и редактирование чужих баркодов и сжигание чужой free-edit квоты.

### Предлагаемый багфикс

Три изменения, все обязательны.

1. Добавить в `.env.example` рядом с остальными секретами, без рабочего значения:

```
# ── Секреты (ОБЯЗАТЕЛЬНЫ, старт без них должен падать) ─────────────────────
# Должен совпадать с JWT_SECRET Auth Service. Сгенерировать: openssl rand -base64 48
JWT_SECRET=
...
Существующие строки `INTERNAL_SERVICE_JWT=dev-internal-token` и `ADMIN_JWT=dev-admin-token` заменить на пустые: рабочее значение в примере копируется в `.env` без раздумий.

2. Пробросить в оба compose-файла:
...
Синтаксис `${VAR:?message}` заставляет `docker compose` отказаться от запуска при незаданной переменной — отказ происходит до старта контейнера.

3. Убрать сам дефолт (это часть фикса E-03, здесь необходима для полноты):

```go
JWTSecret: getEnv(EnvJWTSecret, ""),
```

с проверкой в `Config.Validate()`, описанной в E-17.

### Комментарий

Найдено сверкой `.env.example` с `config.go`, а не чтением кода: в коде всё выглядит штатно — переменная объявлена, документирована комментарием на `:16-17`, читается через общий хелпер. Дефект существует только в промежутке между кодом и документацией, поэтому не обнаруживается ни чтением одного файла, ни тестами. Отношение к E-17: там я писал, что `Config.Validate()` — самая выгодная правка раздела, закрывающая пять дефектов. E-21 уточняет, что её недостаточно: без пункта 2 (проброс в compose) валидация начнёт падать в prod-like режиме даже у оператора, который всё заполнил правильно, поскольку значение не доходит до процесса.
