...
| D-15 | `UNIT_PRICE`: цена в real против коэффициента в mock | Высокая | Да, проба |
...
Вариант 2 — начать использовать в bulk-пути, что заодно улучшает B-10: одна блокировка на батч ��аёт корректную единицу компенсации.
...
## D-15. `UNIT_PRICE` имеет несовместимую семантику в mock и real: цена против коэффициента

**Суть.** Одно и то же значение конфигурации `cfg.UnitPrice` в реальном адаптере — цена за единицу, а в mock-адаптере — коэффициент доступного баланса. При дефолтном `UNIT_PRICE=0.50` mock-биллинг отказывает в генерации одной единицы: `AllowedTotal=0` → `INSUFFICIENT_FUNDS`.

**Место.** `internal/config/config.go:135` (загрузка), `internal/app/api_app.go:127` и `internal/app/worker_app.go:63` (передача в mock), `internal/adapters/billing/mock_client.go:17,24-45`, `internal/adapters/billing/http_client.go:69,129,137` (real).

**Подробное описание.**

Значение читается один раз с дефолтом `0.50`:
...
В реальном адаптере цена за единицу — величина, **приходящая от Billing** в ответе, а не из конфига BFF:

```go
// internal/adapters/billing/http_client.go:69
UnitPrice float64 `json:"unitPrice"`
```

То есть в production `cfg.UnitPrice` в расчётах вообще не участвует — `AllowedTotal` и `unitPrice` присылает Billing.

В mock тот же `cfg.UnitPrice` подаётся в конструктор и трактуется принципиально иначе — как ratio:

```go
// internal/adapters/billing/mock_client.go:24-29 (комментарий автора)
// UnitPrice используется как коэффициент доступного баланса:
//   - UnitPrice <= 0.0  → AllowedTotal = 0 (INSUFFICIENT_FUNDS)
//   - 0.0 < UnitPrice < 1.0 → AllowedTotal = floor(UnitPrice * units)
//   - UnitPrice >= 1.0  → AllowedTotal = units
```

Проба на дефолтном значении из `config.go:135`:

```
PROBE GAP: units=1  -> AllowedTotal=0 Partial=true (ratio-семантика)
PROBE GAP: units=2  -> AllowedTotal=1 Partial=true
PROBE GAP: units=3  -> AllowedTotal=1 Partial=true
PROBE GAP: units=10 -> AllowedTotal=5 Partial=true
PROBE GAP CONFIRMED: units=1 при дефолтном UNIT_PRICE=0.50 -> AllowedTotal=0 => INSUFFICIENT_FUNDS
PROBE GAP: ожидание при price-семантике -> AllowedTotal=1, факт -> 0
```

Последствия. Дефолтная конфигурация из `.env.example` (`UNIT_PRICE=0.50`) в mock-режиме даёт:

- `units=1` — `AllowedTotal=0`, генерация невозможна вообще;
- любой `units` — `Partial=true` **всегда**, потому что `floor(0.5 * units) < units` при любом положительном `units`.

Второе важнее первого: в mock-режиме **каждый** запрос попадает на partial-путь. Значит, разработчик, поднявший сервис по README, наблюдает исключительно поведение `PARTIAL_FUNDS` (C-06) и никогда не видит нормальный полный флоу — а именно этот путь содержит C-11, C-15 и B-03. Одновременно и обратное: полный успешный путь в mock-режиме недостижим без явной переустановки `UNIT_PRICE>=1`, то есть не проверяется вручную никем.

Почему это дефект границы, а не конфигурации: порт `ports.BillingClient` не содержит понятия «цена» — оно скрыто в конструкторах реализаций. Слой сборки (`api_app.go:127`) передаёт `cfg.UnitPrice` в mock, полагая, что смысл совпадает с production, где то же поле не используется. Ни компилятор, ни тесты не могут это поймать: типы совпадают (`float64`), а семантика расходится. Это тот же класс, что D-03 (несовместимые формы ошибок) и D-07 (несовместимая семантика `PendingCount`) — mock и real соблюдают синтаксический контракт и нарушают смысловой.

**Предлагаемый багфикс.** Не переиспользовать production-параметр как тестовый рычаг. Дать mock собственную явно названную настройку:

```go
// internal/adapters/billing/mock_client.go
type MockClient struct {
    // AvailableRatio — доля запрошенных единиц, которую mock считает доступной.
    // 0.0 = нет средств, 1.0 = всё доступно. Не имеет отношения к цене.
    AvailableRatio float64
}

func NewMockClient(availableRatio float64) *MockClient {
    if availableRatio <= 0 {
        availableRatio = 1.0 // по умолчанию mock не мешает разработке
    }
    return &MockClient{AvailableRatio: availableRatio}
}
```

В слое сборки передавать отдельную переменную, а не цену:

```go
// internal/app/api_app.go:127
billingClient = billing.NewMockClient(cfg.MockBillingAvailableRatio) // MOCK_BILLING_RATIO, дефолт 1.0
```

И задокументировать в `.env.example`:

```
# ── Mock billing (только при пустом BILLING_URL) ──────────────────────────
# Доля доступных единиц: 1.0 = полный успех, 0.5 = partial, 0.0 = отказ.
MOCK_BILLING_RATIO=1.0
```

Дефолт `1.0` делает нормальный флоу основным в разработке, а partial — воспроизводимым по явному желанию.

**Комментарий.** Найдено при сверке `.env.example` с кодом, а не при чтении адаптеров: сам mock честно документирует свою ratio-семантику в комментарии `:24-29`, поэтому изолированное чтение файла подозрений не вызывает. Дефект возникает только на стыке трёх мест — `config.go` (дефолт цены), `.env.example` (`UNIT_PRICE=0.50` как «цена») и `mock_client.go` (трактовка как ratio). Прямо связано с E-06 (fail-open биллинг): mock подставляется автоматически при пустом `BILLING_URL`, поэтому эта семантика активна не только в тестах, но и в любом запуске без сконфигурированного Billing.
...
| 7 | **D-11** | Структура конфигурации роутера; вместе с E-07 (��амена shared secret) |
